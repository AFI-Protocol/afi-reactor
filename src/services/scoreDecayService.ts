 ...
 *
 * @param doc - Reactor scored signal document from vault
 ...
 * @example
 * ```typescript
 * const doc = await reactorVault.findOne({ signalId: "sig-123" });
...
export function computeDecayedUwrScore(
  doc: ReactorScoredSignalDocument,
  nowIso: string
): number | null {
  // Require all necessary fields (nested under pipeline)
