...
export async function fetchBaselineSignals(
  ...
  testCollection?: Collection<TssdSignalDocument>
 ...
 */
function mapTssdDocToNoveltyInput(doc: TssdSignalDocument): NoveltySignalInput {
  ...
  // Extract UWR axes from analystScore
  const axes = doc.pipeline.analystScore?.uwrAxes;

  // Extract timestamp from rawUss provenance (canonical source)
  const createdAt = doc.rawUss?.provenance?.ingestedAt || doc.createdAt.toISOString();
