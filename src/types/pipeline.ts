 ...
 *
 * @module afi-reactor/src/types/pipeline
 */
...
export interface PipeheadMetadata {
  ...
  /** Whether pipehead is deprecated */
  deprecated?: boolean;
...
export abstract class BasePipehead implements Pipehead {
  ...
  async execute(state: PipelineState): Promise<PipelineState> {
    ...
    try {
      ...
      // Update the trace entry with completion status
      ...
      // Update the trace entry with failure status
      const endTime = Date.now();
      const endTimeIso = new Date(endTime).toISOString();
      const duration = endTime - startTime;
